...
describe("video resume checkpoint migration", () => {
  it("is authenticated, asset-bound, stale-safe, and cannot grant completion", () => {
    ...
    expect(checkpointFunction).toMatch(/p_checkpoint_sequence bigint/i);
    expect(checkpointFunction).toMatch(/checkpoint_sequence[\s\S]*<> p_checkpoint_sequence/i);
    expect(checkpointFunction).not.toMatch(/client_updated_at/i);
  ...
  it("keeps server time as the observation baseline and uses one server sequence for ordering", () => {
    expect(checkpointFunction).toMatch(
      /last_observed_position_seconds\s*,\s*last_observed_at[\s\S]*?checkpoint_sequence\s*,\s*updated_at/i,
    );
    expect(checkpointFunction).toMatch(
      /least\(p_position_seconds[\s\S]*?\)\s*,[\s\S]*?'\[\]'::jsonb\s*,\s*0\s*,\s*null\s*,\s*v_asset_version\s*,\s*v_checkpoint_sequence\s*,\s*v_now/i,
    );
    expect(checkpointFunction).toMatch(
      /last_observed_at\s*=\s*case[\s\S]*?then null[\s\S]*?updated_at\s*=\s*v_now/i,
    );
    expect(checkpointFunction).toMatch(
      /select checkpoint_sequence\s*,\s*asset_version[\s\S]*?v_stored_asset_version[\s\S]*?distinct from v_asset_version[\s\S]*?v_existing_checkpoint_sequence\s*:=\s*0/i,
    );
    expect(playbackFunction).not.toMatch(/client_updated_at/i);
    ...
    expect(playbackFunction).toMatch(
      /checkpoint_sequence\s*=\s*excluded\.checkpoint_sequence/i,
    );
    expect(playbackFunction).toMatch(
  ...
  it("keeps the race regression and server-sequence stale scenarios in the acceptance test", () => {
    ...
    expect(raceTest).toMatch(/server sequence/i);
