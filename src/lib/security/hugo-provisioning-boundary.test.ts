...
describe("Hugo-only provisioning boundary", () => {
  ...
  it("never writes login status while saving Institute settings", () => {
    ...
    expect(editAction).toContain("createAdminClient");
    expect(editAction).toContain('rpc("fn_update_institute_role"');
    expect(usersAction).toContain("createAdminClient");
    expect(usersAction).toContain('rpc("fn_update_institute_role"');
    expect(editAction).not.toContain('.from("profiles").update(');
    expect(usersAction).not.toContain('.from("profiles").update(');
  });
});
