...
test("combined index binds the approved quizbank review and current changed guide", async () => {
  ...
  assert.equal(index.status, "quiz_approved_guide_pending_human_decision");
...
test("recorded quiz approval stays distinct from the pending guide decision", async () => {
...
test("review index reflects quiz approval without creating guide reacceptance", async () => {
  ...
  assert.match(index.scope_note, /Neither state authorizes import, publication/i);
});
