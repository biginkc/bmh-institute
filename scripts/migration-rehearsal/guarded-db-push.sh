...
# except through the gate: the gate runs first, `set -e` aborts on any non-zero
# exit, and the push is the last line of this file.
#
# It also removes the gate/push target-mismatch hazard. The gate and the push
# are both built from the SAME PG* environment, so they cannot point at
# different databases (checking TEST and writing PRODUCTION).
#
# Usage:
...
# That entry pins the expected Supabase project ref, so a typo'd or swapped
# connection refuses instead of writing to the wrong database.
#
# Exit codes: 0 only if the gate passed AND the push succeeded. Every other
# path is non-zero.
...
for arg in "$@"; do
  case "$arg" in
    ...
    --migrations-dir=*) MIGRATIONS_DIR="${arg#--migrations-dir=}" ;;
    --timeout-ms=*) GATE_TIMEOUT_MS="${arg#--timeout-ms=}" ;;
    --dry-run) DRY_RUN="yes" ;;
    *)
      echo "guarded-db-push: unrecognised argument '$arg'." >&2
      echo "Usage: guarded-db-push.sh --target=<label> [--dry-run] [--migrations-dir=PATH] [--timeout-ms=N]" >&2
      exit 64
      ;;
  esac
done
...
GATE_ARGS=("--target=$TARGET")
[ -n "$MIGRATIONS_DIR" ] && GATE_ARGS+=("--migrations-dir=$MIGRATIONS_DIR")
[ -n "$GATE_TIMEOUT_MS" ] && GATE_ARGS+=("--timeout-ms=$GATE_TIMEOUT_MS")
...
node scripts/migration-rehearsal/check-migration-safety.mjs "${GATE_ARGS[@]}"
...
if [ "$DRY_RUN" = "yes" ]; then
  ...
  exec supabase db push --include-all --db-url "$DB_URL" --dry-run
fi

echo "guarded-db-push: gate passed. Pushing to target '$TARGET'."
exec supabase db push --include-all --db-url "$DB_URL" --yes
