-- Keep role-play completion aligned with the catalog access and lesson-progress
-- contracts. Active owners/admins may exercise content they can actually read;
-- unreleased imported content still requires its explicit reviewer grant.
...
begin
  ...
  if not exists (
    ...
    raise exception 'active learner or authorized administrator is required';
  end if;
...
comment on function public.fn_complete_role_play_block(
  uuid, uuid, text, text, integer, jsonb, jsonb
) is 'Atomically persists one verified Closer Lab result and required block progress after rechecking active user state, exact catalog access, unlock, block, scenario, and replay identity.';
