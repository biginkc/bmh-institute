-- Atomically attach approved media and remove learner transcript links from
-- existing unpublished imported video blocks without replaying the full catalog.
...
begin
  ...
  if (
    ...
    join public.content_blocks block
      on block.id = (patch.value ->> 'block_id')::uuid
