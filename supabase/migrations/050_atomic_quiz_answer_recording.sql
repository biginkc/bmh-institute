-- Persist each quiz answer atomically so concurrent tabs cannot overwrite a
-- different question and a learner cannot change an answer after its reveal.

set lock_timeout = '10s';

create or replace function public.fn_record_quiz_answer(
  p_attempt_id uuid,
  p_question_id uuid,
  p_selected text[]
)
...
begin
  ...
  if coalesce(v_attempt.responses, '{}'::jsonb) ? p_question_id::text then
    ...
    if v_stored is not distinct from v_selected_sorted then
      return query
      select v_attempt.responses, v_attempt.completed_at, true;
      return;
    end if;
  ...
  where attempt.id = p_attempt_id
  returning attempt.responses, attempt.completed_at
    into v_attempt.responses, v_attempt.completed_at;

  return query
  select v_attempt.responses, v_attempt.completed_at, false;
end;
$$;
...
comment on function public.fn_record_quiz_answer(uuid, uuid, text[]) is
  'Atomically records a learner quiz answer once, with idempotent same-answer retries and no cross-attempt existence leak.';
