...
create table public.content_import_video_poster_replacement_records (
  ...
  replacement_payload_sha256 text not null check (replacement_payload_sha256 ~ '^[0-9a-f]{64}$'),
  ...
  unique (import_id, replacement_payload_sha256)
);
...
begin
  if tg_op <> 'INSERT'
    ...
    or coalesce(current_setting('bmh.replace_video_posters_payload_sha256', true), '') <> new.replacement_payload_sha256
  then
  ...
  if v_already_replaced_count = v_replacement_count then
    if exists (
      ...
      where record.import_id = p_import_id
        and record.replacement_payload_sha256 = v_payload_sha256
...
revoke all on function public.fn_replace_released_imported_video_posters(text, jsonb, text, text, text)
from public, anon, authenticated;
grant execute on function public.fn_replace_released_imported_video_posters(text, jsonb, text, text, text)
to service_role;
