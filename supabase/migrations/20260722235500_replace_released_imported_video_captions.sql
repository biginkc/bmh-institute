-- Replace only the checksum-addressed caption path on released imported video
-- blocks. Preserve the immutable release attestation and append an exact
-- correction record instead of rewriting release history.

create table public.content_import_video_caption_replacement_records (
...
comment on table public.content_import_video_caption_replacement_records is
  'Append-only checksum evidence for exact caption-path corrections made after an imported course release.';

alter table public.content_import_video_caption_replacement_records enable row level security;
revoke all on table public.content_import_video_caption_replacement_records
from public, anon, authenticated;
grant select on table public.content_import_video_caption_replacement_records
to service_role;
...
begin
  if tg_op <> 'INSERT'
    ...
    or coalesce(current_setting('bmh.replace_video_captions_import_id', true), '') <> new.import_id
    or coalesce(current_setting('bmh.replace_video_captions_payload_sha256', true), '') <> new.database_payload_sha256
  then
    raise exception 'Imported video caption replacement records are immutable and operation-bound.'
      using errcode = '42501';
  end if;
  return new;
end;
...
revoke all on function public.fn_guard_import_video_caption_replacement_record()
from public, anon, authenticated;

create trigger content_import_video_caption_replacement_records_guard
before insert or update or delete on public.content_import_video_caption_replacement_records
for each row execute function public.fn_guard_import_video_caption_replacement_record();
...
begin
  if coalesce(auth.role(), '') <> 'service_role' then
    raise exception 'Released imported video caption replacement requires service_role.'
      using errcode = '42501';
  end if;
  if p_import_id is null or p_import_id !~ '^[a-z0-9][a-z0-9._-]{0,127}-v[0-9]+$' then
    raise exception 'Released imported video caption replacement has an invalid versioned import_id.'
      using errcode = '22023';
  end if;
  if p_approval_evidence_sha256 is null or p_approval_evidence_sha256 !~ '^[0-9a-f]{64}$' then
    raise exception 'Released imported video caption replacement requires exact approval evidence.'
      using errcode = '22023';
  end if;
  if p_client_payload_sha256 is null or p_client_payload_sha256 !~ '^[0-9a-f]{64}$' then
    raise exception 'Released imported video caption replacement requires the exact client payload checksum.'
      using errcode = '22023';
  end if;
  if p_expected_catalog_sha256 is null or p_expected_catalog_sha256 !~ '^[0-9a-f]{64}$' then
    raise exception 'Released imported video caption replacement requires an exact production preflight.'
      using errcode = '22023';
  end if;
  if p_replacements is null or jsonb_typeof(p_replacements) <> 'array' then
    raise exception 'Released imported video caption replacement payload must be an array.'
      using errcode = '22023';
  end if;

  v_replacement_count := jsonb_array_length(p_replacements);
  if v_replacement_count < 1 or v_replacement_count > 100 then
    raise exception 'Released imported video caption replacement requires 1 to 100 replacements.'
      using errcode = '22023';
  end if;
  ...
  if exists (
    ...
    where jsonb_typeof(replacement.value) <> 'object'
      ...
      or jsonb_typeof(replacement.value -> 'caption_asset_key') <> 'string'
      or replacement.value ->> 'caption_asset_key' !~ '^caption-video-slot-[a-z0-9-]+$'
      or jsonb_typeof(replacement.value -> 'expected_content') <> 'object'
      or jsonb_typeof(replacement.value -> 'expected_content' -> 'caption_path') <> 'string'
      or replacement.value -> 'expected_content' ->> 'caption_path' <>
        replacement.value ->> 'expected_caption_path'
      or jsonb_typeof(replacement.value -> 'expected_caption_path') <> 'string'
      or jsonb_typeof(replacement.value -> 'expected_caption_sha256') <> 'string'
      or jsonb_typeof(replacement.value -> 'replacement_caption_path') <> 'string'
      or jsonb_typeof(replacement.value -> 'replacement_caption_sha256') <> 'string'
      ...
      or (replacement.value ->> 'expected_size_bytes')::numeric > 10485760
      ...
      or (replacement.value ->> 'replacement_size_bytes')::numeric > 10485760
      or replacement.value ->> 'expected_caption_sha256' !~ '^[0-9a-f]{64}$'
      or replacement.value ->> 'replacement_caption_sha256' !~ '^[0-9a-f]{64}$'
      or replacement.value ->> 'expected_caption_sha256' = replacement.value ->> 'replacement_caption_sha256'
      or replacement.value ->> 'expected_caption_path' !~
        '^courses/[a-z0-9-]+/v[0-9]+/captions/[a-z0-9-]+\.[0-9a-f]{64}\.vtt$'
      or replacement.value ->> 'replacement_caption_path' !~
        '^courses/[a-z0-9-]+/v[0-9]+/captions/[a-z0-9-]+\.[0-9a-f]{64}\.vtt$'
      or not starts_with(replacement.value ->> 'expected_caption_path', v_storage_prefix || '/captions/')
      or not starts_with(replacement.value ->> 'replacement_caption_path', v_storage_prefix || '/captions/')
      or replacement.value ->> 'expected_caption_path' <>
        v_storage_prefix || '/captions/' || substring(replacement.value ->> 'caption_asset_key' from 9) ||
          '.' || (replacement.value ->> 'expected_caption_sha256') || '.vtt'
      or replacement.value ->> 'replacement_caption_path' <>
        v_storage_prefix || '/captions/' || substring(replacement.value ->> 'caption_asset_key' from 9) ||
          '.' || (replacement.value ->> 'replacement_caption_sha256') || '.vtt'
  ) then
    raise exception 'Released imported video caption replacement has malformed or noncanonical values.'
      using errcode = '22023';
  end if;
  ...
  if (
    ...
    select count(distinct replacement.value ->> 'caption_asset_key')
    from jsonb_array_elements(p_replacements) replacement(value)
  ) <> v_replacement_count then
    raise exception 'Released imported video caption replacement block IDs and caption keys must be unique.'
      using errcode = '22023';
  end if;
  ...
  then
    raise exception 'Released imported video caption replacement requires the matching published release.'
      using errcode = '42501';
  end if;
  ...
  join public.content_blocks block
   ...
   and block.content = jsonb_set(
     ...
     '{caption_path}',
     to_jsonb(replacement.value ->> 'replacement_caption_path'),
     false
   )
  ...
  if v_already_replaced_count = v_replacement_count then
        ...
        and not exists (
          ...
          where not exists (
            ...
            where object.bucket_id = 'content'
              and object.name = replacement.value ->> 'replacement_caption_path'
              and coalesce(
                to_jsonb(object) -> 'user_metadata' ->> 'sha256',
                object.metadata ->> 'sha256'
              ) = replacement.value ->> 'replacement_caption_sha256'
              and coalesce(
    ...
    raise exception 'Released imported video caption replacement refused: current paths lack the exact audit record.'
      using errcode = '40001';
  end if;
  ...
  if exists (
    ...
    where not exists (
      ...
      where object.bucket_id = 'content'
        and object.name = replacement.value ->> 'expected_caption_path'
        and coalesce(
          to_jsonb(object) -> 'user_metadata' ->> 'sha256',
          object.metadata ->> 'sha256'
        ) = replacement.value ->> 'expected_caption_sha256'
        and coalesce(
          to_jsonb(object) -> 'user_metadata' ->> 'course_import_id',
          to_jsonb(object) -> 'user_metadata' ->> 'courseImportId',
          object.metadata ->> 'course_import_id',
    ...
    raise exception 'Released imported video caption replacement refused: exact old or new storage object is missing.'
      using errcode = '22023';
  end if;
  ...
  if v_prior_catalog_sha256 <> p_expected_catalog_sha256 then
    raise exception 'Released imported video caption replacement refused: catalog drifted from the exact production preflight.'
      using errcode = '40001';
  end if;
  ...
  then
    raise exception 'Released imported video caption replacement refused: catalog drifted after its latest caption correction.'
      using errcode = '40001';
  end if;
  ...
  with replacements as (
    select
      ...
      replacement.value ->> 'replacement_caption_path' as replacement_caption_path
    from jsonb_array_elements(p_replacements) replacement(value)
  )
  ...
  set content = jsonb_set(
    block.content,
    '{caption_path}',
    to_jsonb(replacements.replacement_caption_path),
    false
  )
  ...
  if v_updated_count <> v_replacement_count then
    raise exception 'Released imported video caption replacement refused: target, ownership, type, or expected path mismatch.'
      using errcode = '40001';
  end if;

  v_replacement_catalog_sha256 := public.fn_course_import_catalog_sha256(p_import_id);
  perform set_config('bmh.replace_video_captions_import_id', p_import_id, true);
  perform set_config('bmh.replace_video_captions_payload_sha256', v_payload_sha256, true);
...
revoke all on function public.fn_replace_released_imported_video_captions(text, jsonb, text, text, text)
from public, anon, authenticated;
grant execute on function public.fn_replace_released_imported_video_captions(text, jsonb, text, text, text)
to service_role;
