-- A replacement pool may contain deterministic rows that do not exist yet.
-- Preserve v1's strict existing-row contract and add v2, which permits missing
-- retained rows while still refusing any retained ID found outside this import.

set lock_timeout = '10s';

create or replace function public.fn_prune_replaced_quiz_pools_v2(
  p_import_id text,
  ...
  if cardinality(v_quiz_ids) = 0
    or exists (
      ...
      where question.id = any(v_question_ids)
        and question.quiz_id <> all(v_quiz_ids)
    )
    or exists (
      select 1 from public.answer_options option
      where option.id = any(v_option_ids)
        and option.question_id <> all(v_question_ids)
    )
  then
...
revoke all on function public.fn_prune_replaced_quiz_pools_v2(text, uuid[], uuid[], text)
  from public, anon, authenticated;
grant execute on function public.fn_prune_replaced_quiz_pools_v2(text, uuid[], uuid[], text)
  to service_role;
