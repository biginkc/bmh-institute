...
- Made that capture boundary exclusive with `stopPropagation()` before the document navigation, so no downstream Next Link handler can dispatch a competing client transition.
...
- After Claude A5 rejected `ace90bf` for the missing propagation stop, the corrected exact implementation reran the same canonical TEST setup plus both Chromium flows: 3 of 3 in 57.6 seconds.
...
- Claude A5 then rejected `ace90bf` because the native capture handler prevented the default action but did not stop propagation. That finding was accepted and fixed in `3bf15b6`; the source invariant now requires `preventDefault`, `stopPropagation`, and `location.assign` in that order.
