...
- [x] **HARDEN-06**: Forgot-password and password-reset paths enforce server-side rate limiting (request-count threshold per IP plus per-email window), with a test that asserts the second submission within the threshold is rejected
...
- [x] **TYPE-01**: Supabase types are generated via `supabase gen types` and the generated `Database` type is wired into the Supabase client, with all `as string` / `as number` / `as boolean` assertions on Supabase results removed across the report and lesson pages
...
- [ ] **TEST-03**: Playwright write-path coverage exercises invite acceptance, quiz submission, assignment upload, admin approval and revision, and password reset against the prod-config harness. Deferred to GitHub issue #2 until non-prod Supabase and email capture are available.
...
| TEST-03 | Phase 4 | Deferred to issue #2 |
