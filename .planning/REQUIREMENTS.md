...
- [ ] **TPAR-03**: Production smoke harness covers the HARDEN-01 admin route guard from a learner viewpoint — the existing admin-context `e2e-prod/` setup is extended with a learner-context spec (separate storage state or in-spec fresh sign-in), and `npm run test:prod` runs both contexts against the live deployment with documented credentials
