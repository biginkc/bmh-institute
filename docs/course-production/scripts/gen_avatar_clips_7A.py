...
OUT = "/Users/jarradhenry/Sites/BMH apps/BMH Institute/course-assets/heygen/lesson7A"
AV_CAFE    = "b2cd05454d284058ad8d7303545821e6"   # Cafe Andrea (bookends b01/b13)
AV_HEADSET = "e527528e584a404f9da68ee4faca1353"   # Headset Andrea (corner circles b03/b05/b11)
BLUE = "#62b3f3"
...
# (name, beat-tag, avatar, motion, background)  bg=None keeps the avatar's own setting (cafe)
CLIPS = []
CLIPS.append(("hero_b01_intro", "b01_intro",      AV_CAFE,    "seated at the cafe table, warm and friendly, minimal natural gestures, relaxed hands", None))
CLIPS.append(("circle_b03",     "b03_reframe",    AV_HEADSET, "standing still, hands relaxed at sides, minimal natural gestures, warm reassuring smile", BLUE))
CLIPS.append(("circle_b05",     "b05_fourtypes",  AV_HEADSET, "standing still, hands relaxed at sides, minimal natural gestures, warm reassuring smile", BLUE))
CLIPS.append(("circle_b11",     "b11_sequence",   AV_HEADSET, "standing still, hands relaxed at sides, minimal natural gestures, warm reassuring smile", BLUE))
CLIPS.append(("hero_b13_outro", "b13_outro",      AV_CAFE,    "seated at the cafe table, warm and upbeat, minimal natural gestures, friendly send-off", None))
...
for name, tag, avid, motion, bg in CLIPS:
    ...
    if not c.get("video_id"):
        body = {"type":"avatar","avatar_id":avid,"audio_asset_id":c["audio_asset"],
                "title":f"7A-{name}","resolution":"720p","aspect_ratio":"16:9",
...
for _ in range(90):
    ...
    for name, vid in list(pending.items()):
        ...
        save()
print("7A CLIPS DONE:", sum(1 for c in C.values() if c.get("file")), "/", len(CLIPS), flush=True)
