...
const OUT = path.join(ROOT, "course-assets/heygen/lesson6B/seedance");
const SCENES = path.join(ROOT, "course-assets/scenes/module-06-lesson6B");
const STATE_PATH = path.join(OUT, "_animations.json");
const ONLY = new Set(
  (process.env.LESSON6B_SEEDANCE_ONLY || "")
    .split(",")
    .map((value) => value.trim())
    .filter(Boolean),
);
const FORCE = new Set(
  (process.env.LESSON6B_SEEDANCE_FORCE || "")
    .split(",")
    .map((value) => value.trim())
    .filter(Boolean),
);
...
const STYLE_LOCK = [
  ...
  "Preserve the exact Jarrad-approved still composition, character identities, outfits, faces, props, baked text, and colors.",
  ...
  "Every baked word and punctuation mark must remain perfectly pixel-stable, readable, and unchanged for the entire clip.",
].join(" ");

const NEGATIVE = [
  "NEGATIVE:",
  "photorealism, 3D render, cinematic lighting, shadows, gradients, texture, skin-tone shading, tan skin, peach skin, pink skin,",
  "changed text, shimmering text, repainted text, new text, captions, numbers, logos, watermarks, pseudo-writing, scribbled words,",
...
const BEATS = [
  {
    id: "b03_briefam",
    still: "m06_L6B_b03_briefam.png",
    out: "anim_b03_briefam.mp4",
    prompt:
      "Motion: Sam makes a tiny natural listening idle only, with a very small head settle and subtle breathing. LOCKED STATIC CAMERA. Sam remains exactly one person with the same orange headset, face, hair, orange shirt, cream pants, and yellow shoes. The summary card and its exact baked text DIANE · DAYTON · DUPLEX 11 YRS · BEHIND ON TAXES stay completely frozen and pixel-stable with zero shimmer, repainting, spelling change, punctuation change, or motion. Keep the bottom-right circle pocket entirely empty canonical blue. No hand gesture, no new object.",
  },
  {
    id: "b05_frame",
    still: "m06_L6B_b05_frame.png",
    out: "anim_b05_frame.mp4",
    prompt:
      "Motion: Diane makes one barely perceptible calm shoulder settle while holding the yellow phone handset to her ear, then returns to the exact start pose. ABSOLUTELY LOCKED STATIC CAMERA. The full-body Diane stays the exact same size and exact same centered position for every frame. NO zoom, NO push-in, NO pan, NO crop, NO reframe, NO camera movement of any kind. Exactly one Diane remains on screen with the same gray hair, orange cardigan, cream dress, yellow shoes, calm face, white skin, and one yellow handset. Her identity, fingers, handset, outfit, and face must not morph. No thought bubble, no speech bubble, no prop, no text, no new object.",
  },
];
...
async function init() {
  await rpc("initialize", {
    ...
    capabilities: {},
    clientInfo: {name: "bmh-course-lesson6b-seedance", version: "1.0"},
  });
  const headers = {
    accept: "application/json, text/event-stream",
    authorization: `Bearer ${getToken()}`,
...
async function main() {
  ...
  for (const beat of selected) {
    ...
    if (FORCE.has(beat.id)) {
      entry.attempt = Number(entry.attempt || 1) + 1;
      delete entry.jobId;
      delete entry.file;
      delete entry.url;
  ...
  console.log("6B SEEDANCE DONE", done, "/", selected.length, "deferred", deferred);
...
main().catch((error) => {
  ...
  if (/out of credits|insufficient|balance/i.test(message)) {
    console.error("6B SEEDANCE HALT: INSUFFICIENT HIGGSFIELD CREDITS - STOP AND TELL JARRAD");
    process.exit(2);
  }
  process.exit(1);
});
