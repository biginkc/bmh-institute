...
CALM_BEACH = (
    ...
    "for the entire clip, no raised hands, no waving, no pointing, minimal natural head motion only"
)

CLIPS = [
    ("hero_b01_bridge", "b01_bridge", CALM_BEACH),
    ("hero_b06_disclosure_issues", "b06_disclosure_issues", CALM_BEACH),
    ("hero_b11_next_stop_faq", "b11_next_stop_faq", CALM_BEACH),
]
...
def upload_asset(path):
    boundary = f"----bmh8b{secrets.token_hex(12)}"
...
force = {name.strip() for name in __import__("os").environ.get("FORCE_CLIPS_8B", "").split(",") if name.strip()}
clip_names = {name for name, *_ in CLIPS}

for name, tag, motion_prompt in CLIPS:
    ...
    if name in force:
        for key in ("video_id", "file", "error"):
            clip.pop(key, None)
        clip["forced_reroll_reason"] = "manual reroll"
    ...
    if not clip.get("audio_asset"):
        try:
            ...
            if is_credit_error(msg):
                print("8B AVATAR HALT: INSUFFICIENT CREDITS - STOP AND TELL JARRAD", flush=True)
                raise SystemExit(2)
            raise
...
for _ in range(150):
    ...
    for name, video_id in list(pending.items()):
        ...
        if status == "completed":
            ...
            clips[name]["source_wav_sha256"] = sha256_file(pathlib.Path(state[tag]["wav"]))
            pending.pop(name)
            save(clips_path, clips)
...
print("8B CLIPS DONE:", done, "/", len(CLIPS), flush=True)
if done < len(CLIPS):
    raise SystemExit(1)
