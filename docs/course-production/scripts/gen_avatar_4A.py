...
AV_CAFE    = "b2cd05454d284058ad8d7303545821e6"   # Cafe Andrea (bookends b01/b10)
AV_HEADSET = "e527528e584a404f9da68ee4faca1353"   # Headset Andrea (corner circles b02/b09)
...
CLIPS = []
CLIPS.append(("hero_b01_intro", "b01_intro",     AV_CAFE,    "seated at the cafe table, warm and friendly, minimal natural gestures, relaxed hands"))
CLIPS.append(("circle_b02",     "b02_overview",  AV_HEADSET, "standing still, hands relaxed at sides, minimal natural gestures, warm reassuring smile"))
CLIPS.append(("circle_b09",     "b09_ownership", AV_HEADSET, "standing still, hands relaxed at sides, minimal natural gestures, warm reassuring smile"))
CLIPS.append(("hero_b10_outro", "b10_outro",     AV_CAFE,    "seated at the cafe table, warm and upbeat, minimal natural gestures, friendly send-off"))
    ...
    if not c.get("video_id"):
        try:
            r = api("POST","/v3/videos",{"type":"avatar","avatar_id":avid,"audio_asset_id":c["audio_asset"],
                "title":f"4A-{name}","resolution":"720p","aspect_ratio":"16:9",
...
for _ in range(90):
    ...
    for name, vid in list(pending.items()):
        ...
        save()
print("4A CLIPS DONE:", sum(1 for c in C.values() if c.get("file")), "/", len(CLIPS), flush=True)
